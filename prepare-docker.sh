...
if [ -n "${FROM_RELEASE}" ]; then
  ...
  then
    ...
    echo "Downloading ${ARTEMIS_DIST_FILE_NAME}.asc from ${ARTEMIS_BASE_URL}..."
    curl --progress-bar "${ARTEMIS_BASE_URL}${ARTEMIS_DIST_FILE_NAME}.asc" --output "${CURL_OUTPUT}.asc"

    echo "Downloading KEYS from https://downloads.apache.org..."
    curl --progress-bar "https://downloads.apache.org/activemq/KEYS" --output "${BASE_TMPDIR}/${ARTEMIS_VERSION}/KEYS"

    gpg --no-tty --import "${BASE_TMPDIR}/${ARTEMIS_VERSION}/KEYS"
    gpg --no-tty "${CURL_OUTPUT}.asc"

    echo ""
    echo ""

    echo "Removing ${BASE_TMPDIR}/${ARTEMIS_VERSION}/KEYS..."
    rm "${BASE_TMPDIR}/${ARTEMIS_VERSION}/KEYS"

    echo "Removing ${CURL_OUTPUT}.asc..."
    rm "${CURL_OUTPUT}.asc"
